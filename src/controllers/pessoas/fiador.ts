...
export const CadastrarFiador = async (req: Request, res: Response): Promise<Response> => {
    const { pessoaFisicaId, enderecoId, imovelComoFiancaId, ...fiadorData } = req.body;
  
    if (!pessoaFisicaId || !enderecoId || !imovelComoFiancaId) {
      return res.status(400).json({ message: 'Os IDs da pessoa física, do endereço e do imóvel são obrigatórios!' });
    }
  
    try {
      const fiadorCadastrado = await cadastrarFiador(fiadorData, pessoaFisicaId, enderecoId, imovelComoFiancaId);
...
export const ObterFiadorPorId = async (req: Request, res: Response): Promise<Response> => {
  const { id } = req.params;
...
export const ExcluirFiador = async (req: Request, res: Response): Promise<Response> => {
  const { id } = req.params;
...
export const AtualizarFiador = async (req: Request, res: Response): Promise<Response> => {
  const { id } = req.params;
