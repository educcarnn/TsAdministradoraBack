...
} from "../../services/pessoas/pessoaJuridica";
...
export const CadastrarPessoaJuridica = async (
  ...
  const socioData = req.body.socioData;
  const files = req.files as Express.Multer.File[];

  try {
    const novaPessoaJuridica = await cadastrarPessoaJuridica(
      pessoaJuridicaData,
      socioData,
      files
    );
    ...
    return res
      .status(500)
      .json({ message: "Erro ao cadastrar Pessoa Jurídica" });
  }
};

export const ObterTodasPessoasJuridicas = async (
  _req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Erro ao obter todas as Pessoas Jurídicas" });
  }
};
...
export const DeletarPessoaJuridicaPorId = async (
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Erro ao deletar Pessoa Jurídica por ID" });
  }
};
...
export const AtualizarPessoaJuridicaPorId = async (
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Pessoa jurídica atualizada com sucesso!" });
  } catch (error) {
    console.error("Erro ao atualizar Pessoa Jurídica por ID:", error);
    return res
      .status(500)
      .json({ message: "Erro ao atualizar Pessoa Jurídica por ID" });
  }
};
...
export const ObterPessoaJuridicaPorId = async (
  ...
  try {
    ...
    if (pessoaJuridica) {
      ...
      return res
        .status(404)
        .json({ message: "Pessoa jurídica não encontrada" });
    }
  } catch (error) {
    console.error("Erro ao obter Pessoa Jurídica por ID:", error);
    return res
      .status(500)
      .json({ message: "Erro ao obter Pessoa Jurídica por ID" });
  }
};
