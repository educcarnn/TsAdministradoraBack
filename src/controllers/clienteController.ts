...
export const obterUsuarioPorId = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Consulta para obter usuário por ID de Pessoa Física
    const queryFisica = `
      SELECT * FROM tabela_pessoas_fisicas WHERE id = $1
    `;
    const resultFisica = await client.query(queryFisica, [id]);
    const usuarioFisica = resultFisica.rows[0];

    // Consulta para obter usuário por ID de Pessoa Jurídica
    const queryJuridica = `
      SELECT * FROM tabela_pessoas_juridicas WHERE id = $1
    `;
    const resultJuridica = await client.query(queryJuridica, [id]);
    const usuarioJuridica = resultJuridica.rows[0];

    client.release();

    // Combinar resultados das duas consultas
    const usuario = usuarioFisica || usuarioJuridica;
